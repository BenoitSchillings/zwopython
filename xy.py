...
def mainloop(args):
	...
	while(True):
		...
		vmin = np.min(img)
		vmax = np.max(img)
		print("max ", time.time(), np.mean(img))
...
if __name__ == "__main__":
	...
	parser.add_argument("-exp", "--exp", type=float, default = 0.003, help="exposure in seconds (default 1.0)")
	parser.add_argument("-gain", "--gain", type=int, default = 30, help="camera gain (default 200)")
	parser.add_argument("-count", "--count", type=int, default = 111000, help="number of frames to capture")
	parser.add_argument("-crop", "--crop", type=float, default = 0.75, help="number of frames to capture")
